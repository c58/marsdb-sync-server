...
export default class CollectionManager extends _currentDelegateClass {
  ...
  _remoteInsert(doc, { randomSeed, connection }) {
    this._ensureDocumentId(doc, connection, randomSeed);
    return this.db.insert(doc);
  }

  _remoteUpdate(query, modifier, { randomSeed, connection }) {
  ...
  _ensureDocumentId(doc, connection, randomSeed) {
    if (doc._id && _check.string(randomSeed) && randomSeed.length === 20) {
      const sequenceSeed = [randomSeed, `/collection/${this.db.modelName}`];
      const seededID = Random.createWithSeed.apply(null, sequenceSeed).id(17);
      if (doc._id !== seededID) {
