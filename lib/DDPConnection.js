...
import { PromiseQueue, EJSON, Random } from 'marsdb';
...
export default class DDPConnection extends AsyncEventEmitter {
  constructor(rawConn) {
    ...
    this._heartbeat.on('ping', _bind(this.sendPing, this));
    this._heartbeat.on('pong', _bind(this.sendPong, this));
  ...
  _processMessage(msg) {
    switch (msg.msg) {
      ...
      case 'ping': return this._heartbeat.handlePong(msg);
      case 'method':
      case 'sub':
      case 'unsub':
        if (!this._connected) {
  ...
  _handleConnectMessage(msg) {
    if (!this._connected) {
      this._connected = true;
      this._sessionId = _random.id();
      this.sendMessage({
        msg: 'connected',
